...
export function registerCommands(context: vscode.ExtensionContext) {
  const commands = [
    ...
    {
      command: "uv.upgrade",
      callback: () => runCommand("uv pip install --upgrade"),
    },
...
async function syncDependencies() {
  ...
  const options = await vscode.window.showQuickPick(
    [
      {
        label: "Basic sync",
        description: "Sync dependencies with default options",
      },
      {
        label: "Sync from specific file",
        description: "Sync dependencies from a specific requirements file",
      },
      {
        label: "Sync specific groups",
        description: "Sync specific dependency groups",
      },
    ],
    {
  ...
  if (options.label === "Sync from specific file") {
    ...
    if (files.length === 0) {
      vscode.window.showErrorMessage(
        "No requirements files found in the workspace."
      );
      return;
    }
...
async function upgradeDependencies() {
  ...
  const options = await vscode.window.showQuickPick(
    [
      ...
      {
        label: "Upgrade specific package",
        description: "Upgrade a specific package to its latest version",
      },
    ],
    {
...
async function manageVirtualEnv() {
  ...
  const options = await vscode.window.showQuickPick(
    [
      ...
      {
        label: "Create venv with specific Python",
        description:
...
async function runScript() {
  ...
  const usePythonVersion = await vscode.window.showQuickPick(
    ["Use default Python", "Specify Python version"],
    {
...
async function generateLockFile() {
  ...
  const options = await vscode.window.showQuickPick(
    [
      {
        label: "Basic compile",
        description: "Generate lock file with default options",
      },
      {
      ...
      {
        label: "Specify extras",
        description: "Include specific optional dependencies",
      },
      {
        label: "Specify groups",
        description: "Compile specific dependency groups",
      },
    ],
    {
  ...
  vscode.window.withProgress(
    ...
    async (progress) => {
      try {
        ...
        vscode.window.showErrorMessage(
          `Failed to generate uv.lock file: ${error.message}`
        );
      }
    }
  );
