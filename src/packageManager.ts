...
import { outdatedDecorationType, upToDateDecorationType, getDecorationOptions } from './decorations';
...
// Parse a dependency line from requirements.txt or pyproject.toml
function parsePackageLine(line: string, languageId: string): { name: string, version: string | null } | null {
    line = line.trim();
...
export async function addVersionComparisonDecorations(document: vscode.TextDocument) {
    ...
    const decorationPromises = lines.map((line, i) => limit(async () => {
        ...
        if (parsed) {
            const requirement: ProjectNameRequirement = { name: parsed.name, type: 'ProjectName' };
            try {
                const metadata = await pypi.fetchPackageMetadata(requirement);
                const latestVersion = metadata.info.version;
                const declaredVersion = parsed.version ? semver.coerce(parsed.version)?.version : null;
